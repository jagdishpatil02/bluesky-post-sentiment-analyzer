...
export default function Home() {
  ...
  const [sentiment, setSentiment] = useState<Array<{label: string; score: number}> | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const postRecord = posts.data.posts[0].record as { text: string };
      const sentimentResult = await query({ inputs: postRecord.text });
    ...
    <div className="container mx-auto p-4 max-w-2xl">
      ...
      {post && (
        <div className="border rounded-lg p-4 bg-gray-900 text-white mb-4">
          ...
          {(post.embed as AppBskyEmbedImages.View)?.images && (
            <div className="mt-2">
              {(post.embed as AppBskyEmbedImages.View).images.map((image: { fullsize: string }, index: number) => (
                <img 
                  key={index}
                  src={image.fullsize} 
                  alt="Post image" 
      ...
      {sentiment && (
        <div className="border rounded-lg p-4 bg-white">
          ...
          <div className="space-y-2">
            {sentiment.map((s: {label: string; score: number}) => (
